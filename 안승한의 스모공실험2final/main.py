...
Final submission for SME Experiment2 Final Project.
...
The grader calls main().
main() returns a numpy array with shape (2, num_user).
"""
...
def find_mat_file() -> str:
    candidates = ["DH_FR1.mat", "InF_DH_FR1.mat", "_InF_DH_FR1.mat"]
...
class HuberIRWLSSolver:
    ...
    def solve_ls(self, distance_vector: np.ndarray, weights: np.ndarray | None = None, x0: np.ndarray | None = None) -> np.ndarray:
        ...
        if weights is None:
            ...
            weights = np.maximum(np.asarray(weights, dtype=float).reshape(-1), 1e-12)
            sqrt_w = np.sqrt(weights)

        def residual(pos):
            predicted_distance = np.linalg.norm(
                self.bs_positions.T - pos.reshape(1, 2),
                axis=1,
            )
...
def apply_gate(delta: np.ndarray, gate_config: dict) -> np.ndarray:
    mode = gate_config.get("gate_mode", "constant")
    scale = float(gate_config.get("global_scale", 1.0))

    if mode == "constant":
        return scale * delta

    # The final selected model uses constant gate. This fallback keeps the code safe
    # if a different gate config is stored in model.pkl.
    return scale * delta
...
def main():
    ...
    gate_config = model_data["gate_config"]
    ...
    gated_delta = apply_gate(delta, gate_config)
    p_hat = p_huber + gated_delta
